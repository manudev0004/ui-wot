...
export class StatusIndicator {
    ...
    static getStatusClasses(status) {
        ...
        if (status === 'success')
            return `${base} opacity-100 rounded-full bg-green-500 text-white`;
        if (status === 'error')
            return `${base} opacity-100 rounded-full bg-red-500 text-white`;
        return `${base} opacity-0`;
    }
    /** Apply status with auto-clear timers */
    static applyStatus(target, status, errorMessage) {
        if (!target)
            return;
    ...
    static renderStatusBadge(status, error, h) {
        if (!h)
            return null;
    ...
    static renderTimestamp(lastUpdated, theme, h) {
        if (!h)
            return null;
