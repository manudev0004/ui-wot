...
export function GroupContainer({
  ...
  if (group.options.hideWrapper) {
    return (
      <div className="group-container-content-only relative">
        {/* Floating toggle to show wrapper */}
        <button
          onClick={(e) => {
            e.stopPropagation();
            handleHideWrapperChange(false);
          }}
          className="absolute top-2 right-2 z-50 bg-blue-500 hover:bg-blue-600 text-white p-1 rounded shadow-lg text-xs transition-colors"
          title="Show group wrapper"
        >
          📦
        </button>
