...
export function SmartEditPopup({
  component,
  affordance,
  onClose,
  attributesList,
  ...
  const calculateOptimalPosition = useCallback(() => {
    ...
    if (spaceRight >= popupWidth) {
      ...
      top = Math.max(16, Math.min(
        componentRect.top + scrollY,
        viewportHeight - popupHeight - 16 + scrollY
      ));
      ...
      left = Math.max(16, Math.min(
        componentRect.left + scrollX,
        viewportWidth - popupWidth - 16 + scrollX
      ));
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (popupRef.current && !popupRef.current.contains(event.target as Node)) {
        ...
        if (targetElement.closest(`[data-component-id="${component.id}"]`)) {
          return;
        }
        onClose();
      }
    ...
    <>
      ...
      >
        ...
        <div className="smart-popup-content">
          ...
          <div className="smart-popup-section">
              ...
              <input
                ...
                onChange={e => {
                  ...
                  dispatch({
                    ...
                    payload: { id: component.id, updates: { title: next } }
                  });
                  onAttributeChange(component.id, 'label', next);
                }}
            ...
            <div className="smart-popup-field">
              ...
              <select
                ...
                onChange={e => onVariantChange(component.id, e.target.value)}
            ...
            <div className="smart-popup-field">
              <label className="smart-popup-checkbox-label">
                <input
                  type="checkbox"
                  checked={component.hideCard || false}
                  onChange={e => {
                    dispatch({
                      type: 'UPDATE_COMPONENT',
                      payload: { id: component.id, updates: { hideCard: e.target.checked } }
                    });
                  }}
                ...
                {attributesList.map(name => {
                  ...
                  return (
                    <div key={name} className="smart-popup-field">
                      <label className="smart-popup-label">
                      ...
                      {attrType === 'boolean' ? (
                        <div className="smart-popup-checkbox-wrapper">
                          <input
                            ...
                            onChange={e => onAttributeChange(component.id, name, e.target.checked ? 'true' : 'false')}
                            className="smart-popup-checkbox"
                          />
                          <span className="smart-popup-checkbox-status">
                        ...
                        <input
                          type="number"
                          value={currentValue}
                          onChange={e => onAttributeChange(component.id, name, e.target.value)}
                          className="smart-popup-input smart-popup-input--small"
          ...
          <div className="smart-popup-section">
            <button
              onClick={() => onComponentClose(component.id)}
              className="smart-popup-remove-btn"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
