...
export function AffordanceSelectionPage() {
  ...
  const handleAffordanceToggle = (affordanceKey: string) => {
    setSelectedAffordances(prev =>
      prev.includes(affordanceKey)
        ? prev.filter(key => key !== affordanceKey)
        : [...prev, affordanceKey]
    );
  };
  ...
  const handleLoad = async () => {
    ...
    try {
      ...
      const components: WoTComponent[] = selectedAffordances.map((affordanceKey, index) => {
            ...
            >
              <div className="p-4">
                ...
                {affordance.description && (
                  <p className="text-sm text-gray-600 mb-3">{affordance.description}</p>
                )}

                <div className="flex items-center justify-between">
                  <span
                    className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getComponentBadgeColor(
