...
function slugifyTitle(title) {
  return String(title || 'thing').toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim().replace(/\s+/g, '');
}
...
const registry = new Map(); // slug -> ExposedThing
      ...
      req.on('end', async () => {
        try {
          ...
          function stripForms(obj) {
            ...
            if (Array.isArray(obj)) { obj.forEach(stripForms); return; }
...
main().catch((e) => { console.error('[td-host] fatal:', e); process.exit(1); });
